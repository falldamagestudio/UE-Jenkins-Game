...
function Get-InstalledEpicGamesLauncherEngines {
	...
	try {
		$ManifestLocation = Join-Path $env:ProgramData -ChildPath "Epic\UnrealEngineLauncher\LauncherInstalled.dat" -ErrorAction Stop

		$Manifest = Get-Content -Raw -Path $ManifestLocation -ErrorAction Stop | ConvertFrom-Json
...
function Get-InstalledLocallyRegisteredEngines {
	...
	try {
		...
		$LocallyRegisteredEngines = (Get-Item -Path $LocallyRegisteredEnginesRegistryLocation -ErrorAction Stop).Property | Where { $_ -ne $null } | ForEach-Object { [PSCustomObject] @{
				Id = $_
				Path = (Get-ItemProperty -Path $LocallyRegisteredEnginesRegistryLocation -ErrorAction Stop).$_
			}
		}
...
function Get-EngineAssociationForProject {
	...
	try {
		...
		$UProject = Get-Content -Raw -Path $UProjectLocation -ErrorAction Stop | ConvertFrom-Json
